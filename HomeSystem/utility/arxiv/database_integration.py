...
class EnhancedArxivTool(ArxivTool):
    """增强的ArXiv工具，集成数据库功能"""
    
    def __init__(self, search_host: str = None, enable_database: bool = True):
        super().__init__(search_host)
        self.enable_database = enable_database
        self.db_manager = ArxivDatabaseManager() if enable_database else None
    
    def arxivSearch(self, query: str, num_results: int = 20, sort_by: str = "relevance",
                   order: str = "desc", max_results: int = None, kwargs: dict = None,
                   use_direct_api: bool = True, skip_processed: bool = False) -> ArxivResult:
        """
        增强的搜索方法，支持自动去重和数据库存储
        
        :param skip_processed: 是否跳过已处理的论文
        """
        # 调用原始搜索方法
        results = super().arxivSearch(
            query, num_results, sort_by, order, max_results, kwargs, use_direct_api
        )
        
        if not self.enable_database or not results.results:
            return results
        ...
        if skip_processed:
            ...
            for paper in results.results:
                if paper.arxiv_id and not self.db_manager.is_processed(paper.arxiv_id):
                    filtered_results.append(paper)
            
            # 创建新的结果对象
            if len(filtered_results) != len(results.results):
                filtered_dict_results = []
                for paper in filtered_results:
                    ...
                    filtered_dict_results.append(result_dict)
                results = ArxivResult(filtered_dict_results)
                logger.info(f"过滤后剩余 {results.num_results} 篇未处理论文")
        
        # 自动保存到数据库
        try:
            saved_count = self.db_manager.batch_save_papers(results)
            if saved_count > 0:
                logger.info(f"自动保存了 {saved_count} 篇新论文到数据库")
        except Exception as e:
            logger.error(f"自动保存论文失败: {e}")
        
        return results
    
    def process_paper(self, arxiv_data: ArxivData, processor_func: Callable[[ArxivData], Any]) -> Any:
        """
        处理论文并自动更新状态
        
        :param arxiv_data: 论文数据
        :param processor_func: 处理函数
        :return: 处理结果
        """
        if not self.enable_database:
            return processor_func(arxiv_data)
        
        try:
            # 检查是否已处理
            if arxiv_data.arxiv_id and self.db_manager.is_processed(arxiv_data.arxiv_id):
                logger.info(f"论文已处理，跳过: {arxiv_data.arxiv_id}")
                return "已处理"
            
            # 执行处理函数
            result = processor_func(arxiv_data)
            
            # 标记为已处理
            if arxiv_data.arxiv_id:
                self.db_manager.mark_processed(arxiv_data.arxiv_id, 'completed')
                logger.info(f"论文处理完成: {arxiv_data.arxiv_id}")
            ...
            # 标记为失败
            if arxiv_data.arxiv_id:
                self.db_manager.mark_processed(arxiv_data.arxiv_id, 'failed')
    ...
    def get_unprocessed_papers(self, limit: int = 10) -> List[ArxivPaperModel]:
        ...
        return self.db_manager.get_unprocessed_papers(limit)